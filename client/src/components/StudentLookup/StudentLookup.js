...
class StudentLookup extends React.Component {
    ...
    handleFetchStudent(){
        ...
        fetch(getStudentEndpoint+`?id=${this.state.name}&queryType=${this.state.queryType}`)
          .then(res => res.json())
          .then(
            (result) => {
                if (result.length > 0) {
    ...
    render() {
        ...
        return (
            <div className="student-lookup__container">
                ...
                <div className={this.state.loading === 'loading' ? 'student-lookup__show student-lookup__loading' : 'student-lookup__hide'}>
                    <Spinner />
                </div>
                <div className={this.state.loading === 'no-results' ? '' : 'student-lookup__hide'}>No search results. Try again</div>
                <div className={this.state.loading === 'loading' ? 'student-lookup__search-results student-lookup__hide' : 'student-lookup__search-results'}>
